        ...
        public async Task CreateAddressAsync()
        {
            ...
            try
            {
                if(listingId != null && countryId != null && stateId != null && cityId != null && areaId != null && pincodeId != null && localityId != null && !string.IsNullOrEmpty(address))
                {
                    try
                    {
                        BOL.LISTING.Address add = new BOL.LISTING.Address
                        {
                            ...
                            City = cityId.Value,
                            AssemblyID = areaId.Value,
                            PincodeID = pincodeId.Value,
                            LocalityID = localityId.Value,
                            LocalAddress = address
                        };

                        await listingContext.AddAsync(add);
                ...
                {
                    // Show notification
                    await NoticeWithIcon(NotificationType.Error, NotificationPlacement.BottomRight, "Error", $"Country, State, City, Area, Pincode, Locality and Address is Compulsory.");
