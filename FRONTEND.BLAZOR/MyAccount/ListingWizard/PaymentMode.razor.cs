...
namespace FRONTEND.BLAZOR.MyAccount.ListingWizard
{
    public partial class PaymentMode
    {
        ...
        // Begin: Create Payment Mode
        public async Task CreatePaymentModeAsync()
        {
            buttonBusy = true;

            // Check if specialisation already exists
            var duplicate = await listingContext.PaymentMode.Where(i => i.ListingID == listingId).FirstOrDefaultAsync();
            ...
            try
            {
                if (duplicate == null)
                {
                    try
                    {
                        ...
                        if(propertyCount > 0)
                        {
                            BOL.LISTING.PaymentMode pm = new BOL.LISTING.PaymentMode
                            {
                            ...
                            await listingContext.AddAsync(pm);
                        ...
                        {
                            // Show notification
                            await NoticeWithIcon(NotificationType.Error, NotificationPlacement.BottomRight, "Error", "Please select at least one payment mode.");
                ...
                {
                    // Show notification
                    await NoticeWithIcon(NotificationType.Success, NotificationPlacement.BottomRight, "Error", $"Payment Mode for Listing ID {duplicate.ListingID} already exists.");
        ...
        protected async override Task OnInitializedAsync()
        {
            try
            {
                ...
                if (user.Identity.IsAuthenticated)
                {
                    ...
                    {
                        string url = "/MyAccount/ListingWizard/PaymentModeEdit/" + listingId;
