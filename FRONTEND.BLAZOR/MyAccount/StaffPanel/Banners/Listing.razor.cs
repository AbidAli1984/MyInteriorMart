...
namespace FRONTEND.BLAZOR.MyAccount.StaffPanel.Banners
{
    public partial class Listing
    {
        ...
        // Begin: Get All Listing Banners
        public IEnumerable<ListingBanner> ListListingBanners { get; set; }
        public async Task GetAllListingBannersAsync()
        {
            ListListingBanners = await listingContext.ListingBanner
                .OrderBy(i => i.Placement)
                .ThenBy(i => i.Priority)
                .ToListAsync();
        }
        // End: Get All Listing Banners
        ...
        public async Task CreateAndUploadBannerAsync()
        {
            try
            {
                ...
                if (Placement != null && Name != null && Priority != null && LinkUrl != null && TargetWindow != null && FileBanner != null && FirstCategoryID != null && SecondCategoryID != null && ThirdCategoryID != null)
                {
                    ...
                    {
                        // Add Listing Banner Record in Database
                        ListingBanner lm = new ListingBanner
                        {
                            Placement = Placement,
                            Name = Name,
                        ...
                        await listingContext.AddAsync(lm);
                        ...
                        // Create Listing Directory in Cloud Box If Does Not Exists
                        var categoryDirPath = cloudBoxPath + "\\listing";
                        if (!Directory.Exists(categoryDirPath))
                        {
                            Directory.CreateDirectory(categoryDirPath);
                        }

                        // Get Listing Banner ID
                        var listingBannerId = lm.BannerId;

                        // Rename Listing Banner File
                        var newFileName = listingBannerId + "." + fileExt;
                        ...
                        var fileUploadPath = cloudBoxPath + "\\listing\\" + newFileName;
                        ...
                        // Get All Listing Banners
                        await GetAllListingBannersAsync();

                        // Show Success Message
                        SuccessMessage = $"Banner {lm.Name} uploaded successfully.";
                    }
                }
                else
                {
        ...
        public async Task DeleteBannerAsync(int bannerId)
        {
            try
            {
                var banner = await listingContext.ListingBanner
                .FindAsync(bannerId);
                ...
                if (banner != null)
                {
                    ...
                    var deleteImageFilePath = cloudBoxPath + "\\listing\\" + bannerId + ".jpeg";
