        ...
        public async Task PopulateListFLVM()
        {
            if (level == "fc")
            {
                ...
                foreach (var item in listFLVM)
                {
                    ...
                    // Begin: Get Second Category Name
                    int listingId = item.Listing.ListingID;

                    var secondCatId = await listingContext.Categories
                        .Where(i => i.ListingID == listingId)
                        .Select(i => i.SecondCategoryID)
                        .FirstOrDefaultAsync();

                    string secondCatName = await categoriesContext.SecondCategory
                        .Where(i => i.SecondCategoryID == secondCatId)
                        .Select(i => i.Name)
                        .FirstOrDefaultAsync();
                    // End: Get Second Category Name
